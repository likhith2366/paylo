...
### 2.4 Card Tokenization & the Vault (the piece that actually determines your PCI scope)

This is the single most important architectural decision for anything touching card numbers, and it was under-specified earlier — worth designing properly, not just mentioning in passing.

**The core principle:** the raw card number (PAN) should touch **as few systems as physically possible** — ideally exactly one, small, isolated service — and never touch your main application servers, your Payments API, your logs, or the merchant's servers at all. This single decision is what determines whether you're in scope for full PCI DSS (expensive, slow, audited) or a much lighter **SAQ A** scope (because you never handle raw card data yourself).
...
Merchant's checkout page
        │
        │  embeds an <iframe> that YOU host (not the merchant, not their JS)
        ▼
┌─────────────────────────────┐
│  Hosted card-input iframe     │  ← served from a dedicated domain, e.g. js.payflow.com
│  (your payflow.js creates    │     The merchant's page never has JS access to the raw
│   and injects this iframe)   │     input field's value — it's sandboxed by browser
└──────────────┬───────────────┘     same-origin rules, exactly like Stripe Elements.
               │
               │  raw card number goes STRAIGHT from the browser to the Vault —
               │  it never transits through the merchant's server, and never
               │  transits through your own main API/application servers either
               ▼
┌─────────────────────────────┐
│   Vault / Tokenization        │  ← a tiny, deliberately minimal, heavily locked-down
│   Service                     │     service. This is the ONLY component in your
│                                │     entire system that ever sees a raw PAN.
│  - encrypts PAN with KMS       │
│    immediately on receipt      │
│  - stores it very briefly      │
│    (or not at all — see below) │
│  - returns an opaque token     │
│    e.g. "tok_1a2b3c4d5e"       │
└──────────────┬───────────────┘
               │
               │  only the TOKEN comes back to the browser
               ▼
Browser hands the token to the merchant's page (via a JS callback,
same pattern as Stripe: merchant's own JS never sees the PAN, only the token)
               │
               ▼
Merchant's server calls YOUR Payments API with the token:
   POST /v1/charges { amount: 5000, currency: "usd", payment_token: "tok_1a2b3c4d5e" }
               │
               ▼
Payments API (§4) never sees a raw card number at all — it just passes the
token through to the Vault (or the Vault decrypts it only at the final moment,
in memory, to submit to the Bank Simulator/processor) and processes as normal.
```

**Key design points:**

- **The iframe is the PCI-scope firewall.** Because it's served from your domain and the merchant's page can't reach into it (browser same-origin policy), the merchant's website — and its own JavaScript — never has access to the raw card number, even though it visually appears "on their page." This is exactly why real Stripe Elements/Checkout, Braintree Hosted Fields, and Adyen's Drop-in all use this same iframe pattern — it's not an accident, it's the entire PCI-scope-reduction strategy.
- **"Temporary vault" is the right instinct — implement it as short-lived, encrypted-at-rest, single-use.** When the Vault receives a raw PAN: encrypt immediately with a KMS-managed key, store just long enough to complete the current authorization (seconds to minutes), then either delete it or let it expire via TTL. If you want to support saved cards ("keep this card on file" for repeat merchants/subscriptions), that's a longer-lived encrypted record in the Vault, but the principle is identical — it's still the only place raw PAN data lives, ever, and it's still returned to the rest of your system only as an opaque token.
- **The Vault is a separate deployable, separate network segment, minimal-privilege.** In AWS terms: its own VPC subnet with the tightest possible security group (only the checkout-iframe endpoint and the Bank Simulator submission path can reach it — not your Payments API, not your Ledger service, not your dashboard), its own IAM role, its own KMS key, and ideally its own small codebase that gets far more scrutiny/review than the rest of the system precisely because it's the one place a real breach would actually matter.
- **Your Payments API, Ledger, and everything else operate purely on tokens.** This is what actually shrinks your compliance burden — if the Ledger service is never able to see a raw card number even in principle, it's out of PCI scope entirely, regardless of how much money flows through it.
- **For the portfolio project**: build the Vault as a real, separate microservice with its own encryption and the iframe pattern — this is genuinely buildable without any real bank/processor relationship, and it's one of the most technically impressive pieces you can demo (show that even inspecting your own Payments API's logs/database, the raw card number is nowhere to be found — only tokens).

### 2.5 API Protocol: REST, gRPC, or GraphQL — different answers for different surfaces

This isn't a single decision — different client-server boundaries in this system have genuinely different needs.

| Boundary | Protocol | Why |
|---|---|---|
| **Public Payments API** (merchant's server → your Payments API, e.g. `POST /v1/charges`) | **REST over HTTPS, JSON** | This is the one third-party developers actually integrate against, so it optimizes for *their* experience, not raw performance: every language has an HTTP+JSON client, it documents cleanly with OpenAPI/Swagger (generates SDKs automatically), and REST's resource model (`/charges`, `/refunds`, `/customers`) maps naturally onto how merchants think about the domain. This mirrors reality — Stripe's actual public API is REST-style, not GraphQL, for exactly these reasons. The `Idempotency-Key` header (§4) is also a natural fit for HTTP semantics in a way that doesn't map as cleanly onto a GraphQL mutation |
| **Merchant Dashboard ↔ your backend** | **REST** (can reuse the same versioned API, or a thin internal BFF layer) | Consumed via TanStack Query from the React dashboard (§2.2) — no need for a second protocol here; keep it simple |
| **Internal service-to-service** (Payments API → Ledger Service → Idempotency Service → Vault → Fraud Scoring) | **gRPC + Protobuf** | This traffic never leaves your infrastructure, so you optimize for what actually matters internally: binary encoding + HTTP/2 multiplexing is meaningfully faster and lower-latency than JSON/REST, which matters because several of these calls sit directly on the synchronous charge hot path (§25.1's latency budget). Protobuf schemas also give you a strongly-typed contract across services — including across languages, which matters directly if you build the Rust idempotency hot-path (§2.1) or the Python-based ML fraud service (§14.3) alongside your Go services; gRPC is genuinely the standard glue for exactly this kind of polyglot microservice mesh |
| **Checkout widget → Vault** (§2.4) | **A single minimal REST/HTTPS endpoint** (`POST /vault/tokenize`) | It's one narrow-purpose call from the browser — no need for gRPC's complexity here, and browsers don't speak gRPC natively anyway (would need grpc-web plus a proxy, unnecessary overhead for one endpoint) |
| **Your webhook deliveries → merchant's server** (§7) | **HTTP POST, signed JSON payload** | Not really a "request/response API" at all — it's a one-way, at-least-once notification, which is exactly why it's built on the async webhook infra rather than being an API call in the traditional sense |
| **Dashboard live transaction feed** (optional nice-to-have) | **Server-Sent Events (SSE)** | If you want the dashboard to show new transactions appearing in real time without polling, SSE is simpler than WebSockets for this one-directional (server→client) use case and works over plain HTTP |

**Why not GraphQL anywhere here:** GraphQL shines when clients need flexible, client-driven queries over read-heavy, deeply-nested data (e.g., a mobile app picking exactly which fields it needs across many related objects). A payments API is dominated by **mutations that move money** — and for those, REST's explicit-endpoint-per-action model plus HTTP's native idempotency/header semantics is a better fit than GraphQL's single-endpoint mutation style. It's a legitimate, defensible choice to skip GraphQL entirely here — you're not missing out on anything the domain actually needs.
...
- Tokenize card data via the dedicated Vault + hosted-iframe pattern (§2.4) — the Payments API, Ledger, and every other service should only ever see opaque tokens, never a raw PAN. This is the actual mechanism that keeps your PCI DSS scope small.
...
16. **The Card Vault + hosted-iframe tokenization service** (§2.4) — a small, deliberately isolated microservice, its own encryption, its own network segment. This is arguably the single most "impressive in an interview" piece to demo: show that a raw card number is provably nowhere in your Payments API's database or logs — only in the Vault, as ciphertext, and everywhere else it's just an opaque token.
